...
public class PizzaController {
    ...
    @PostMapping
    public ResponseEntity<PizzaDTO> cadastrar(@RequestBody @Valid PizzaDTO dto, UriComponentsBuilder uriComponentsBuilder){
        PizzaDTO pizzaDTO = pizzaService.criarPizza(dto);
        URI endereco = uriComponentsBuilder.path("/pizzas/{id}").buildAndExpand(pizzaDTO.getId()).toUri();
